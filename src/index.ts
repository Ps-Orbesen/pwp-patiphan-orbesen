...
const mailgun = new Mailgun(formDate)
...
const validation = [
    ...
    check('message', 'A message of up to 2000 characters is required.').trim().escape()
...
const handlePostRequest = (request: Request, response: Response) => {
    ...
    if (request.recaptcha.error) {
        return response.send(
            `<div class='alert alert-danger' role='alert'><strong>Oh Snap!</strong> There was a 
                Recaptcha error. Please try again. </div>`
        )
    }
    ...
    if (errors.isEmpty() === false) {
        ...
        return response.send(
            `<div class='alert alert-danger' role='alert'><strong>Oh Shoot!</strong> 
            ${currentError.msg} </div>`
        )
    }
    ...
    mailgunClient.messages.create(process.env.MAILGUN_DOMAIN, mailgunData)
        ...
        .catch((errors: any) =>
         response.send(
        `<div class='alert alert-danger' role='alert'><strong>Oh Shoot!</strong> 
           Email failed. Please Try Again </div>`
        ))
